...
class M_cetakwp extends CI_Model
{
	...
	function get_alternatif()
	{
		...
		//$hariini = date('Y-m-d');
		$id_kasus = $this->input->get('id_kasus');
		$hsl = $this->db->query("SELECT tb_kasus.*,tb_alternatif.* FROM 
		tb_kasus,tb_alternatif WHERE tb_alternatif.id_kasus=tb_kasus.id_kasus  and tb_kasus.id_kasus='$id_kasus'");
		return $hsl;
	}
	...
	function get_atributwp()
	{
		...
		//$hariini = date('Y-m-d');
		$id_kasus = $this->input->get('id_kasus');
		$hsl = $this->db->query("SELECT tb_normalisasikreteriawp.*,tb_normalisasi_atribut_wp.*,tb_kreteria.*,tb_kasus.*,tb_alternatif.*,tb_atribut.* FROM 
		tb_kreteria,tb_kasus,tb_alternatif, tb_atribut,tb_normalisasi_atribut_wp,tb_normalisasikreteriawp 
		WHERE tb_alternatif.id_kasus=tb_kasus.id_kasus and tb_normalisasikreteriawp.id_kreteria=tb_kreteria.id_kreteria and 
	...
	function get_hasilwp()
	{
		...
		//$hariini = date('Y-m-d');
		$id_kasus = $this->input->get('id_kasus');
		$hsl = $this->db->query("SELECT tb_alternatif.nm_alternatif,tb_nilaiwp.nilai_wp FROM tb_nilaiwp,tb_alternatif WHERE tb_nilaiwp.id_alternatif=tb_alternatif.id_alternatif and tb_alternatif.id_kasus='$id_kasus' ORDER BY tb_nilaiwp.nilai_wp DESC ");
		return $hsl;
	}
