...
{
	include('config.php');
	$query  = "SELECT id_kreteria FROM kriteria WHERE id_kasus='$id_kasus' ORDER BY id_kreteria";
	$result = mysqli_query($koneksi, $query);

	while ($row = mysqli_fetch_array($result)) {
		$listID[] = $row['id_kreteria'];
	}
...
{
	include('config.php');
	$query  = "SELECT id_alternatif FROM alternatif where id_kasus='$id_kasus' ORDER BY id_alternatif";
	$result = mysqli_query($koneksi, $query);
...
{
	include('config.php');
	$query  = "SELECT nm_kreteria FROM kriteria WHERE id_kasus='$id_kasus' ORDER BY id_kreteria";
	$result = mysqli_query($koneksi, $query);

	while ($row = mysqli_fetch_array($result)) {
		$nama[] = $row['nm_kreteria'];
	}
...
{
	include('config.php');
	$query  = "SELECT nm_alternatif FROM alternatif WHERE id_kasus='$id_kasus' ORDER BY id_alternatif";
	$result = mysqli_query($koneksi, $query);

	while ($row = mysqli_fetch_array($result)) {
		$nama[] = $row['nm_alternatif'];
	}
...
{
	include('config.php');
	$query  = "SELECT count(*) FROM alternatif where id_kasus='$id_kasus'";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$jmlData = $row[0];
	}
...
function getJumlahKriteria()
{
	...
	$query  = "SELECT count(*) FROM kriteria where id_kasus = '$id_kasus'";
	//echo "coba isi $id_kasus";
	$result = mysqli_query($koneksi, $query);
	while ($row = mysqli_fetch_array($result)) {
		$jmlData = $row[0];
	}
...
function deleteKriteria($id)
{
	...
	$query 	= "DELETE FROM kriteria WHERE id_kreteria=$id";
...
function showTabelPerbandingan($jenis, $kriteria)
{
	...
	if ($kriteria == 'kriteria') {
		...
		while ($row = mysqli_fetch_array($result)) {
			$pilihan[] = $row['nm_kreteria'];
		}
	} else {
		$n = getJumlahAlternatif();
		$query = "SELECT nm_alternatif FROM $kriteria WHERE id_kasus='$id_kasus' ORDER BY id_alternatif";
		$result	= mysqli_query($koneksi, $query);
		if (!$result) {
			echo "Error koneksi database!!!";
			exit();
